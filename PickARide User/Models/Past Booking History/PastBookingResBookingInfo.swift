...
class PastBookingResBookingInfo : Codable {
    ...
    let status : TripCompletedStatus?
    let subTotal : String?
    let tax : String?
    let tips : String?
    ...
    var fullName: String {
        [customerFirstName, customerLastName].compactMap({$0}).joined(separator: " ")
    }
    
    var pickupCoordinate: CLLocationCoordinate2D? {
        if let latitude = Double(pickupLat ?? ""), let longitude = Double(pickupLng ?? "") {
    ...
    var dropOffCoordinate: CLLocationCoordinate2D? {
        if let latitude = Double(dropoffLat ?? ""), let longitude = Double(dropoffLng ?? "") {
           return CLLocationCoordinate2D(latitude: latitude, longitude: longitude)
        } else {
            return nil
        }
    ...
    required init(from decoder: Decoder) throws {
        ...
        status = try? values.decode(TripCompletedStatus.self, forKey: .status)
...
enum TripCompletedStatus: String, Codable {
    case canceled
    case accepted
    case pending
    case completed
